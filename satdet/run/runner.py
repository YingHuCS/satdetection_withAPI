...
from .hooks import (Hook, LrUpdaterHook, CheckpointHook, IterTimerHook,
                    OptimizerHook, lr_updater_hook,  DistOptimizerHook, DistSamplerSeedHook)
from .logger import LogBuffer, get_priority, TensorboardXHook, TensorboardXBuffer
from utils import (is_str, is_list_of, mkdir_or_exist, get_dist_info, get_host_info, get_time_str, 
                   obj_from_dict, Config, load_checkpoint, save_checkpoint, symlink)
...
class Runner(object):
        ...
        for i, data_batch in enumerate(data_loader):
            ...
            if 'log_vars' in outputs:
                self.log_buffer.update(outputs['log_vars'],
                                       outputs['num_samples'])
                # self.tensorboardX_buffer.update(outputs['log_vars']) # ying
    ...
    '''
    This function is implemented by ying
    '''
    def val_ying(self):
        pass
        '''
        ...
        cfg = Config.fromfile('/satdet/configs/faster_rcnn_x101_64x4d_fpn_1x_dota.py')
        class_names = get_classes('dota')
        in_dir = '/mmdetection/data/DOTA-v1.0/1024_splitted_train/images/'
        score_thr = 0.5

        img_names = os.listdir(in_dir)[0:1]
        imgs_with_boxes = np.empty([1, 3, 832, 832]) 
        for i, name in enumerate(img_names):
            in_img_path = os.path.join(in_dir, name)
            result = inference_detector(self.model, in_img_path, cfg, device='cuda:7')
            img = cv2.imread(in_img_path)
            for j, item in enumerate(result):
                if len(item) != 0:
                    label = class_names[j]
                    for sub_item in item:
                        if sub_item[-1] >= score_thr:
                            x1, y1 = sub_item[0], sub_item[1]
                            x2, y2 = sub_item[2], sub_item[3]
                            cv2.rectangle(img, (x1,y1), (x2,y2), (0,255,0), 2)
                            cv2.putText(img, label, (x1,y1), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(255,255,255), 1,cv2.LINE_AA)
            imgs_with_boxes[i] = img.transpose(2,0,1)
        self.tensorboardX_buffer.update(dict(imgs_with_boxes=imgs_with_boxes))
        self.call_hook('after_val_epoch')
        '''
            ...
            for i, flow in enumerate(workflow):
                ...
                for _ in range(epochs):
                    if mode == 'train' and self.epoch >= max_epochs:
                        return
                    # epoch_runner(data_loaders[i], **kwargs) # ying
                    if mode == 'train': # ying
                        epoch_runner(data_loaders[i], **kwargs) # ying
                    else: # ying
                        epoch_runner() # ying
    ...
    def register_lr_hooks(self, lr_config):
        if isinstance(lr_config, LrUpdaterHook):
            ...
            # from .hooks import lr_updater_hook
            hook_name = lr_config['policy'].title() + 'LrUpdaterHook'
            if not hasattr(lr_updater_hook, hook_name):
                raise ValueError('"{}" does not exist'.format(hook_name))
            hook_cls = getattr(lr_updater_hook, hook_name)
    ...
    def register_training_hooks(self,
        ...
        self.register_hook(self.build_hook(DistOptimizerHook(**optimizer_config), OptimizerHook))
        self.register_hook(self.build_hook(checkpoint_config, CheckpointHook))
        self.register_hook(IterTimerHook())
        self.register_hook(DistSamplerSeedHook())
        self.register_hook(TensorboardXHook()) # ying
